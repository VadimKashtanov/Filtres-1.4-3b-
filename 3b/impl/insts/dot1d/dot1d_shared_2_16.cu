	...
	float * locd)
{
	...
	FOR(0, d, X/BLOQUE) {
		__partage__x[thy][thx] = x[(depart+_t)*( X ) + DEPART_x +d*BLOQUE + thx];
	...
	float * dp)
{
	...
	//FOR(0, d, T/BLOQUE) {
		...
		__partage__p[thy][thx] = x[(depart+(d*BLOQUE_MAX+thy))*X+DEPART_x +_x];//p[_y*(X+1) + d*BLOQUE + thy];
		__syncthreads();
